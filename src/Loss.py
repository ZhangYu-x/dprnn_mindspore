...
class loss(nn.Cell):
    ...
    def cal_loss(self, source, estimate_source, source_lengths):
        ...
        mean_loss = 0 - self.mean(max_snr)
        reorder_estimate_source = self.reorder_source(estimate_source, perms, max_snr_idx)
        return mean_loss, max_snr, estimate_source, reorder_estimate_source
...
if __name__ == "__main__":
    ...
    estim_source = Tensor(np.random.randn(2, 2, 32000), dtype=mindspore.float32)
    loss, snr, estim_source, reorder_estim_source = \
        my_loss(padded_source, estim_source, mixture_lengths)
    print("_" * 100)
    print(loss)
    print(loss.shape)
    print(snr.shape)
    print(estim_source.shape)
    print(reorder_estim_source.shape)
